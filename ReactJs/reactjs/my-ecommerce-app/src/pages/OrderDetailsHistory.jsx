            ...
            <div className="max-w-7xl mx-auto">
                ...
                >
                    <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--accent-color)] to-emerald-500">
                        Детали заказа #{order.orderNumber || 'Не указан'}
                    </h2>
                ...
                >
                    ...
                    <Tilt tiltMaxAngleX={10} tiltMaxAngleY={10} perspective={1000}>
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-semibold text-[var(--accent-color)] mb-6">
                                Информация о заказе
                            </h3>
                            ...
                            <p className="text-gray-300 mb-3">
                                ...
                                >
                                    {order.status === 'REFUSED' ? 'Отклонён' : order.status === 'PENDING' ? 'В обработке' : 'Груз доставлен'}
                                </span>
                            </p>
                            {order.status === 'REFUSED' && order.reasonRefusal && (
                                <p className="text-red-300 mb-3">
                                    <strong>Причина отказа:</strong> {order.reasonRefusal}
                                </p>
                            )}
                            <p className="text-gray-300">
                                <strong>Адрес доставки:</strong> {order.deliveryAddress || 'Не указан'}
                            </p>
                            ...
                            {order.insurance != null && (
                                <p className="text-gray-300">
                                    <strong>Страховка:</strong> {order.insurance ? 'Да' : 'Нет'}
                                </p>
                            )}
                            {order.promocode && (
                                <p className="text-gray-300">
                    ...
                    <Tilt tiltMaxAngleX={10} tiltMaxAngleY={10} perspective={1000}>
                        ...
                        >
                            ...
                            {isSelfPickup ? (
                                ...
                                <>
                                    <p className="text-gray-300 mb-3">
                                        <strong>Сумма товаров:</strong> ¥{totalItemsPrice.toFixed(2)}
                                    </p>
                                    {totalChinaDeliveryPrice > 0 && (
                                        <p className="text-gray-300 mb-3">
                                            <strong>Доставка по Китаю:</strong> ¥{totalChinaDeliveryPrice.toFixed(2)}
                                        </p>
                                    )}
                                    {order.shippingCost > 0 && (
                                        <p className="text-gray-300 mb-3">
                                            <strong>Стоимость доставки:</strong> ¥{order.shippingCost.toFixed(2)}
                                        </p>
                                    )}
                                    {order.customsDuty > 0 && (
                                        <p className="text-gray-300 mb-3">
                                            <strong>Таможенная пошлина:</strong> ¥{order.customsDuty.toFixed(2)}
                                        </p>
                                    )}
                                    {order.supplierCost > 0 && (
                                        <p className="text-gray-300 mb-3">
                                            <strong>Стоимость поставщика:</strong> ¥{order.supplierCost.toFixed(2)}
                                        </p>
                                    )}
                                    {order.insuranceCost > 0 && (
                                        <p className="text-gray-300 mb-3">
                                            <strong>Стоимость страховки:</strong> ¥{order.insuranceCost.toFixed(2)}
                                        </p>
                                    )}
                                    {order.userDiscountApplied > 0 && (
                                        <p className="text-green-400 mb-3">
                                            <strong>Скидка пользователя:</strong> -¥{order.userDiscountApplied.toFixed(2)}
                                        </p>
                                    )}
                                    {order.discountValue > 0 && (
                                        <p className="text-green-400 mb-3">
                                            <strong>
                                                Скидка по промокоду ({order.discountType === 'PERCENTAGE' ? `${order.discountValue}%` : 'Фиксированная'}):
                                            </strong>{' '}
                                    ...
                                    <div className="mt-6 pt-4 border-t border-gray-600">
                                        ...
                                        <p className="text-3xl font-bold text-[var(--accent-color)] mt-2">
                                        ...
                                        >
                                            ...
                                            {item.purchaseStatus && (
                                                <p className="text-gray-300 mt-1">
                                                    <strong>Статус покупки:</strong>{' '}
                                                    {item.purchaseStatus === 'PENDING' ? 'В обработке' : item.purchaseStatus}
                                                </p>
                                            )}
                                            {item.purchaseRefusalReason && (
                                                <p className="text-red-300 mt-1">
                                                    <strong>Причина отказа:</strong> {item.purchaseRefusalReason}
                                                </p>
                                            )}
                                        </motion.div>
                                    </Tilt>
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-gray-300 mt-1">
                                                        x{item.quantity || 1} • ¥{(item.priceAtTime || 0).toFixed(2)}
                                                    </p>
                                                    {item.chinaDeliveryPrice > 0 && (
                                                        <p className="text-gray-300 mt-1">
                                                            Доставка по Китаю: ¥{item.chinaDeliveryPrice.toFixed(2)}
                                                        </p>
                                                    )}
                                                    {item.purchaseStatus && (
                                                        <p className="text-gray-300 mt-1">
                                                    ...
                                                    {item.trackingNumber && (
                                                        <p className="text-gray-300 mt-1">
                                                            <strong>Трек-номер:</strong> {item.trackingNumber}
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="w-full bg-gray-600 rounded-full h-2 mt-3">
                                                <motion.div
                                                    ...
                                                    animate={{
                                                        width: `${
                                                            (item.quantity / Math.max(...order.items.map((i) => i.quantity || 1), 1)) *
                                                            100
                                                        }%`,
                                                    }}
                                                    transition={{ duration: 1, ease: 'easeInOut' }}
