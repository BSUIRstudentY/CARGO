...
function CartPage() {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      await confirmOrder(deliveryAddress);
  ...
  useEffect(() => {
    const isValidPromo = promoCode.toLowerCase() === 'cargo10' || promoCode.toLowerCase() === 'freedelivery';
    setPromoApplied(isValidPromo);
  }, [promoCode]);

  const handleQuantityChange = (productId, value) => {
    // Разрешаем временно стереть всё или ввести любое значение
    setLocalQuantities(prev => ({
      ...prev,
      [productId]: value === '' ? '' : value, // Пустое значение допускается во время редактирования
    }));
  };

  const handleQuantityBlur = async (productId) => {
    const inputValue = localQuantities[productId] || '';
    ...
    if (inputValue !== '') {
      ...
      if (!isNaN(parsedValue) && parsedValue > 0) {
        ...
        // Если некорректное значение, возвращаем исходное количество
        newQuantity = originalQuantity;
      }
    } else {
      // Пустое поле возвращает исходное количество
      newQuantity = originalQuantity;
    }
    ...
    // Отправляем запрос только если новое значение отличается от исходного и валидно
    if (newQuantity !== originalQuantity) {
      ...
      try {
        ...
        // Откатываем к исходному значению из cart при ошибке
        setLocalQuantities(prev => ({
          ...prev,
  ...
  /*if (loading) return <div className="text-center mt-10 text-white animate-pulse">Загрузка корзины...</div>;
  if (error) return <div className="text-center mt-10 text-red-500">Ошибка: {error}</div>;
  if (!cart || cart.length === 0) return <div className="text-center mt-10 text-gray-400">Корзина пуста</div>;*/
  ...
  const finalTotal = total - discount + insuranceCost;
          ...
          >
            ...
            <div className="flex items-center space-x-4">
              <input
                type="number"
                min="1"
                value={localQuantities[item.productId] !== '' ? localQuantities[item.productId] : ''}
      ...
      <div className="mt-8 p-6 bg-gray-800 rounded-xl border-l-4 border-[var(--accent-color)] shadow-xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="mb-4">
              <label className="flex items-center space-x-3 p-2 bg-gray-700 rounded-lg border border-gray-600 hover:bg-gray-600 transition-all duration-300">
                ...
                <span className="text-sm text-gray-200">Страховка груза (5%)</span>
              </label>
            </div>
          ...
          <div className="text-right">
            ...
            {discount > 0 && <p className="text-sm text-green-500 mb-2">Скидка: -¥{discount.toFixed(2)}</p>}
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="bg-gray-800 p-6 rounded-xl shadow-2xl max-w-md w-full border-l-4 border-[var(--accent-color)]">
            ...
            <p className="mb-2 text-lg">Сумма: ¥{finalTotal.toFixed(2)}</p>
            <p className="mb-4 text-lg">Адрес: {deliveryAddress}</p>
